...
const ForgotPasswordPage: React.FC = () => {
  ...
  const handleEmailSubmit = async (data: EmailFormData) => {
    ...
    try {
      ...
      toast({
        title: 'Error',
        description: error instanceof Error ? error.message : 'Failed to send reset code. Please check your connection and try again.',
        variant: 'destructive',
      });
    } finally {
      setIsLoading(false);
  ...
  if (step === 'otp') {
    return (
      <AuthLayout title="Enter verification code" subtitle="We've sent a 6-digit code to your email">
        ...
        <Button onClick={handleOtpVerify} className="w-full" size="lg" disabled={otp.length !== 6 || isLoading}>
          {isLoading ? (
            <>
