...
const AuthLayout: React.FC<AuthLayoutProps> = ({ children, title, subtitle, rightTop }) => {
  return (
      ...
      <div className="flex-1 flex flex-col justify-center py-12 px-4 sm:px-6 lg:px-12 xl:px-20 bg-background">
        ...
        <div className="w-full max-w-md mx-auto">
          <div className="mb-8 flex items-start justify-between">
            <div>
              {/* <img src={logo} alt="NGS" className="h-[150px] w-auto mb-4" /> */}
              <img
                src={logo}
                alt="NGS"
                className="mx-auto mb-4 h-24 sm:h-28 md:h-32 lg:h-36 w-auto"
              />

              <h2 className="text-3xl font-display font-bold text-foreground">{title}</h2>
              {subtitle && <p className="mt-2 text-muted-foreground">{subtitle}</p>}
            </div>
            {rightTop && <div className="ml-4 flex-shrink-0">{rightTop}</div>}
          </div>
